...
- If we do k-means, what should the k be?? Evaluate the relationship between the value of k and cluster Entropy
...
This paper used the parsed Bitcoin data. The data was already pre-processed, and clustered from address graph to user graph.\
They tried three unsupervised learning methods, which are k-means, Mahalanobis distance based method, and unsupervised SVM. They applied the three methods on the user graph as well as the transaction graph.
For k-means, they evaluated the relationship between value of k and 
